...
*	USE jkcmpfil INDEX jkcmpfil IN 1 ALIAS tblCompany 
	...
	*--> KMC 01/21/2015
	minRoyActive = .F.
	if file (cDataLoc+"min_royalty.dbf")
		use min_royalty in 0 order dlr_code
		rptRefDate = gomonth(ctod(alltrim(str(mBillMonth)) + "/01/" + alltrim(str(mBillYear))),1)-1
		minRoyActive = .T.
		minRoyAmt = 0  
	endif	
	*-->
...
*!*		go top
	...
	* KMC 06/07/2006 select company_no, iif(empty(alltrim(apply_fran)),dlr_code,apply_fran) as dlr_code, 
	...
	*--> worked on (02/10/2012) to allow percent split of revenue for a cust trx between two franchisees.
	select company_no, iif(len(alltrim(apply_fran))=6,apply_fran,dlr_code) as dlr_code, ;
		...
		sep_inv, trx_amt, trx_class, trx_no, trx_tax, trx_type, apply_to ;
	from jkcustrx ;
...
*	USE tmpsprd index tmpsprd alias tblSpread
	...
	*--> KMC 01/21/2015
	if minRoyActive = .T.
		set rela to dlr_code into min_royalty addi
	endif
	*-->	
	...
	DO WHILE .not. EOF() 
		...
		* setting the defaults....
		store 0 to ;
	        ...
	        f_le_pst, f_sPst, cust_sPst && For Canadian Regions
		...
		store 0 to f_tech_tax  && KMC 05/13/2013 - Added to accomodate tax variable associated with Franchisee Trx's coded 'I'.

		store 0 to f_afr_totl, f_afr_acct  && KMC 07/24/2014 
		*----------------------------
...
function CalcRebate(rebateElig)
	...
	IF rebateElig
		if used('AFRebPct') && 02/16/2007 - KMC - Added to allow alternate Acct Fee Rebate Calculations.
			curTable=Select()
			sele AFRebPct
			...
			*-->
			...
			DO CASE
				*--> KMC 07/24/2014 - changed m.f_totl to m.f_afr_totl (next 4 instances).
				CASE between(m.F_afr_Totl,Amt1beg,Amt1end)
  					mRebatePercent = RebPct1
				CASE between(m.F_afr_Totl,Amt2beg,Amt2end)
  					mRebatePercent = RebPct2
				CASE between(m.F_afr_Totl,Amt3beg,Amt3end)
  					mRebatePercent = RebPct3
				CASE m.F_afr_Totl>Amt4beg
			...
			DO CASE
				*--> KMC 07/24/2014 - changed m.f_totl to m.f_afr_totl (next 4 instances).
				CASE between(m.F_afr_Totl,25000.01,45000.00)
  					mRebatePercent = .5
				CASE BETWEEN(m.F_afr_Totl,45000.01,65000.00)
  					mRebatePercent=1.0
				CASE BETWEEN(m.F_afr_Totl,65000.01,85000.00)
  					mRebatePercent=1.5
				CASE m.F_afr_Totl > 85000.00
  					mRebatePercent=2.0
				OTHERWISE 
  					mRebatePercent=0
			ENDCASE
		...
		*mAcctFee = m.f_acct + m.f_atax
		mAcctFee = m.f_afr_acct + m.f_atax
		*-->
		
		*--> KMC 07/24/2014
		*mRebateAmt = round(m.F_Totl * (mRebatePercent / 100),2)
		mRebateAmt = round(m.F_afr_Totl * (mRebatePercent / 100),2)
		*-->
...
function generateFranCalc
	...
	mFNote2 = 0
 	...
 	if tblDlrFil.pct_flag = "Y"
    	f_acct = round(f_totl*(tblDlrFil.add_pct / 100),2)
  		...
  		*--> KMC 07/24/2014
  		f_afr_acct = round(f_afr_totl*(tblDlrFil.add_pct / 100),2)
  		*-->
	...
	*--> KMC 06/30/2008 - Only charge bond if BPPAdmin is set to 'Y'.
 	*if tblDlrFil.Business=0
  	if tblDlrFil.Business=0 or tblDlrFil.BppAdmin<>"Y"
 	 	f_bond=0
  	else
	  	do case
	...
	if eof()   	  	
	 	...
	 	*--> KMC 01/30/2015
 		if minRoyActive
 			...
 			if !eof('min_royalty')
		 		do case
	 				Case !empty(min_royalty.MinRoyDat2) and rptRefDate => min_royalty.MinRoyDat2  
					
						*--> KMC 04/07/2016 - To account for when Royalty total is a negative number (it will happen if credits exceed billings).
		 				*if f_royal < min_royalty.min_roy2
 						*	f_royal = min_royalty.min_roy2
 						*	minRoyAmt = f_royal
 						*endif
						minRoyAmt = min_royalty.min_roy2
						if f_royal < min_royalty.min_roy2 and f_royal => 0
 							f_royal = minRoyAmt
 						endif
 						*-->

	 				Case !empty(min_royalty.MinRoyDat1) and rptRefDate => min_royalty.MinRoyDat1 	

						*--> KMC 04/07/2016 - To account for when Royalty total is a negative number (it will happen if credits exceed billings).
		 				*if f_royal < min_royalty.min_roy1
 						*	f_royal = min_royalty.min_roy1
 						*	minRoyAmt = f_royal
 						*endif
						minRoyAmt = min_royalty.min_roy1
						if f_royal < min_royalty.min_roy1 and f_royal => 0
 							f_royal = minRoyAmt
 						endif
 						*-->
 	...
 	do while mDlrCode = tblCusTrx.dlr_code
		...
		*do while mAryCusTot(lIdx,_RPT_CUSTOMER_NO) = tblCusTrx.cust_no 
		do while mAryCusTot(lIdx,_RPT_CUSTOMER_NO) = tblCusTrx.cust_no and tblCusTrx.dlr_code=mDlrCode 
 			...
 			* perform calculations based on transaction class... 			
 			DO CASE
        		*wait window "Dlr_Code "+mDlrCode+" Cust_No "+mAryCusTot(lIdx,_RPT_CUSTOMER_NO)+" Trx Class "+trx_class+" Royalty "+alltrim(str(tblCusfil.royalty,5,2)) 
	       		CASE trx_class = "B"
  			...
  			*f_royal = f_royal + ((trx_amt * (mCustRoyaltyPct / 100)) * multiply)
   			if trx_class="S" and tblCusTrx.Royalty=0  && 04/23/2012 KMC - ?? Per Jill, Royalty is charged on all customer transactions.
   				* No Royalty on Client Supplies
   			else
   				f_royal = f_royal + ((trx_amt * (mCustRoyaltyPct / 100)) * multiply)
      		endif
     		*-->
    		...
    		*-->
     		...
     		*--> KMC 07/24/2014
     		if tblCusFil.afr_elig = "Y"
     			f_afr_totl = f_afr_totl +(trx_amt * multiply)
      		endif
      		*-->
	...
	if FOUND()
    	do while .not. eof() .and. tblCusFil.cust_no = tblFF.cust_no
		    ...
		    ff_amt = 0
      		ff_dp = .f.
      		do case
      			case tblFF.calc_fact $ "SFZ"  && KMC 03/25/2021 - Added 'Z' 
		        	...
		        	*--> KMC 06/27/2012
		        	*if tblFF.ff_pybill = tblFF.ff_pytotl and tblff.ff_balance = 0
		        	*	ff_num = "Paid"
		        	*endif	
		        	*-->
          ...
          *--> 03/12/2002 KMC - Code altered so old leases that were taxed month by month
    	    ...
    	    *--> are set to 0.        
       		if pymnt_bill = 0
        		...
        		if mTaxAll = "Y"
					...
					*lpay_tax = round(tblLeaseFil.paymnt_amt * (tblLeaseFil.Paymnt_Tax / 100),2) * tblLeaseFil.Pymnt_adv
					*-->
				...
				*--> KMC 12/14/2010 - Added to accomodate for new payment structure of 16 and 28 payments with double final pymt.
        		*--> Payment number on report is +1 of payment number in table, take that into account to determine when to display FP.
        		*--> KMC 08/05/2011 - Updated code to accomodate 40 payment leases too.
        		if (pymnt_bill = 14 and pymnt_totl = 16) or (pymnt_bill = 26 and pymnt_totl = 28) ;
        			or (pymnt_bill = 38 and pymnt_totl = 40)
	        		lpay_num = "FP of " + alltrim(str(PYMNT_totl))
   		     		lpay_amt = tblLeaseFil.paymnt_amt * 2

   		     		*--> KMC 12/22/2010
   		     		lpay_tax = round((tblLeaseFil.paymnt_amt * 2) * (tblLeaseFil.Paymnt_Tax / 100),2)
   		     		*lpay_tax = round(tblLeaseFil.paymnt_amt * (tblLeaseFil.Paymnt_Tax / 100),2) * 2
        			*-->
        	...
        	if "$" $ cDataLoc
      		...
      		*--> KMC 04/13/2011 - Flag to identify when to append asterisk to lease number to identify that FP will be due next month.
			if (pymnt_bill = 13 and pymnt_totl = 16) or (pymnt_bill = 25 and pymnt_totl = 28) ;
      			or (pymnt_bill = 37 and pymnt_totl = 40)
				xAppendAsterisk = .T.
			else
				xAppendAsterisk = .F.
			endif		
			...
			*-->
	        ...
	        repla dlr_code with mDlrCode
      		...
      		*--> KMC 04/13/2011 - Append asterisk to description to identify that Final Payment will be due next month.
		    *repla descripton with tblLeaseFil.descripton
  			if xAppendAsterisk = .T.
		        repla descripton with "* " + tblLeaseFil.descripton
      		else
		        repla descripton with tblLeaseFil.descripton
     		endif
        	...
        	if eof()
				...
				*--> KMC 12/14/2010 - Added to accomodate new payment structure of 16 and 28 payments with double final pymt.
        		*--> At pymnt 14 of 16 or 26 of 28, double payment is pulled, at that point, lease is satisfied.
        		*--> KMC 08/05/2011 - Updated code to address payments with a term of 40.
        		if (tblLeaseFil.pymnt_bill = 14 and tblLeaseFil.pymnt_totl = 16) or (tblLeaseFil.pymnt_bill = 26 and tblLeaseFil.pymnt_totl = 28) ;
					or (tblLeaseFil.pymnt_bill = 38 and tblLeaseFil.pymnt_totl = 40)        			
					do case
						case tblLeaseFil.pymnt_bill = 14 
							repla tran_num with 16
						case tblLeaseFil.pymnt_bill = 26
							repla tran_num with 28
						case tblLeaseFil.pymnt_bill = 38
							repla tran_num with 40
					endcase		
				else
				*--> KMC 12/14/2010
					replace tran_num with iif(tblLeaseFil.pymnt_bill=0,tblLeaseFil.pymnt_adv,tblLeaseFil.pymnt_bill + 1)
				endif
	    	...
	    	*-->   	     
      		...
      		*--> is put on hold for that month, and the Fran report re-created, the post record would still be
...
FUNCTION generateFranRegion
 	...
 	*--> KMC 03/12/2013 - f_tech_tax not stored anywhere because there in no tax on that.
 	seek mDlrCode + "I" 
  	typex = "Technology Fee"
  	do MiscReport2 with "I",f_tech_fee,f_tech_tax
...
function updateSpreadSheet
	...
	lCusfil2 = SELECT()
  	...
  	ro_deduct = ROUND(f_royal + f_rtax + f_acct + f_atax + f_comm_1_in + f_1tax + ;
                ...
                f_note + f_sec_note + f_ff_down + f_ffd_tax + f_tech_fee,2)  && 05/04/2012 - Added Technology Fee.
	...
	*--> KMC 01/16/2012
	NoCBReserve=cDataLoc+"NoCBRes.txt"
	if tblDlrFil.add_pct=5 and file(NoCBReserve) && 3% Acct fee + 2% No CB fee and file NoCBRes in regions datafolder.
		repla no_cb_res with round(t_admin*.20,2) && No CB Reserver, which is 1/2 of the 2% No CB Fee, which equals 20% of the 5% Acct Fee.
		repla t_deduct with t_deduct - no_cb_res && KMC 01/26/2012 - Reduce DRO amount by the No CB Reserve amount.
	else
		repla no_cb_res with 0.00
	endif
	*-->
  	...
  	*--> KMC 07/24/2014 - Replaced next 2 instances of m.f_totl with m.f_afr_totl.
  	if eof() and mRebateAmt > 0
		appe blank
	   	repl company_no with mSysOffice,dlr_code with mDlrCode, ;
	    	...
	    	GrossRev with m.f_afr_totl, AcctFee with mAcctFee, ;
	   ...
	   repl tblDlrFil.RebBal with tblDlrFil.RebBal + mRebateAmt
	    ...
	    repl GrossRev with m.f_afr_totl, AcctFee with mAcctFee, ;
	         RebateAmt with mRebateAmt, RebatePerc with mRebatePercent
	    sele tblDlrFil
...
function generateFranTotals
	...
	*repla bp_admin2 with mBa2
	if tblDlrfil.BPPAdmin = "Y"
		repla bp_admin1 with mBa1
		repla bp_admin2 with mBa2
	else
		repla bp_admin1 with 0
		repla bp_admin2 with 0
	endif				
	*-->
    ...
    do while mIdx <= pArraySize    
		...
		*-->
	    ...
	    repla royalty WITH mRoyalty  && KMC 02/04/2005
  		...
  		repla afr_elig with tblCusFil.afr_elig  && KMC 07/24/2014
