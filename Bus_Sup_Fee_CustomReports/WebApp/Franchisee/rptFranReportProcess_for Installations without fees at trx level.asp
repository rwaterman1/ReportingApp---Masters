...
dim royalty,acctFeePercent,techFeePercent,plantype,address,datesign,business
...
sql = sql & "business,sec_pymnt,sec_note,sec_pybill,sec_pytotl,ad_cur,ded_tech,tech_pct"	
...
if noRecords then
	...
	if trim(oRs("ded_tech")) = "Y" then
		techFeePercent = oRs("tech_pct")
		techFeePercent = cdbl(techFeePercent)/100
	end if	
...
function doMainPageBody
	...
	dim lAccountingFee,lTechnologyFee
	...
	lTechnologyFee = round((cdbl(lSubTotal)*cdbl(techFeePercent)),2)
	lRegDeduct     = aryTmp2(ROY_TOT)+lAccountingFee+lTechnologyFee+cdbl(franPayment)+cdbl(secPayment)
	...
	doMainPageRow "Accounting Fee",formatnumber(lAccountingFee,2),"",""
	doMainPageRow "Technology Fee",formatnumber(lTechnologyFee,2),"",""
