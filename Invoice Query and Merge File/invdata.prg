...
USE cDataLoc+'jkctlfil' IN 0
LOCA FOR control_no=1
mBillMon=jkctlfil.fn_lsmon
mBillYr=jkctlfil.fn_lsyear

cond = " INLIST(a.item_class,'B','E','O','I','S')"

WAIT WINDOW "Retrieving Records..." NOWAIT

USE cDataLoc+'jkinvfil' IN 0
SELECT jkinvfil
SET FILTER TO bill_mon = mBillMon AND bill_year = mBillYr
GO TOP
IF EOF()
	WAIT CLEAR
	MESSAGEBOX("Cannot Proceed Without Monthly Invoice Record!",48,"Alert")
	RETURN
ENDIF
...
SELE b.dsp_name, b.rem_addr, b.rem_city, b.rem_state, b.rem_zip, b.phone,&mgst, ;
	...
	a.inv_no ;
	FROM cDataLoc+'jkarofil' a,cDataLoc+'jkcmpfil' b,cDataLoc+'jkdlrfil' c,cDataLoc+'jkcusfil' d;
	WHERE a.company_no = b.company_no AND ;
	a.dlr_code = c.dlr_code AND ;
	a.cust_no = d.cust_no AND ;
	&cond AND ;
	a.bill_mon = mBillMon AND ;
	a.bill_year = mBillYr  ;
	INTO CURSOR invoice0
...
SELE b.*, ;
	...
	SPACE(10) AS ppsttax, ;
	SPACE(10) AS ptotal ;
	FROM cDataLoc+'jkcustrx' a,invoice0 b ;
	WHERE a.inv_no = b.inv_no AND ;
	a.trx_amt + a.trx_tax > 0 ;
	ORDER BY &cOrder ;
	INTO DBF invdata
